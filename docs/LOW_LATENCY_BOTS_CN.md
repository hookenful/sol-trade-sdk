...
策略需要在买入提交前拒绝危险 mint 时，可以给 `TradingClient` 安装 `TradeRiskGate`。风险门在基础参数校验之后、交易构建和提交之前同步执行。

远程风险数据应由后台任务获取并原子发布为本地不可变快照。缓存缺失和过期时必须显式选择 fail-open 或 fail-closed；不要在风险门中请求 RPC 或审计 API。

用途、API 行为、完整缓存示例和热路径要求见[买入前风险门使用指南](PRE_BUY_RISK_GATE_CN.md)。
